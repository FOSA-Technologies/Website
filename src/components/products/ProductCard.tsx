...
const STATUS_ON_MEDIA: Record<Product['status'], string> = {
...
function ProductMedia({ product }: { product: Product }) {
  ...
  const media = (
    <>
      ...
      <span
        className={`absolute right-3 top-3 inline-flex items-center gap-1.5 rounded-[6px] border px-2.5 py-1 text-[10.5px] font-semibold uppercase tracking-[0.08em] backdrop-blur-[2px] ${STATUS_ON_MEDIA[product.status]}`}
      >
      ...
      <span className="absolute inset-x-0 bottom-0 flex items-center gap-2.5 p-3.5">
        ...
        <span className="text-[13px] font-semibold text-white/90">Voir la démo</span>
      </span>
    </>
  )
  ...
  return (
    <a
      ...
      aria-label={`Voir la démo de ${product.name}`}
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-line bg-white transition-all duration-300 hover:-translate-y-1 hover:border-fosa-300 hover:shadow-card">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-2 text-[14.5px] leading-relaxed text-ink">{product.description}</p>
